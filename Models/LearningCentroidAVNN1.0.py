...
class AutoLambdaAVNN(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, k=5, lr=1e-3, epochs=300, batch_size=256,
                 ...
                 mahalanobis=False, mahal_reg=1e-6,
                 lam_init=0.7, auto_lambda=True, lam_search_step=0.05,
                 n_lam_splits=5, lam_floor=0.5,
        ...
        self.n_lam_splits        = n_lam_splits
    ...
    class _Net(nn.Module):
        ...
        def __init__(self, centroids, K, n_feat, per_class_tau=False, eps=1e-7):
            ...
            # Centroids as LEARNABLE parameters — backprop moves them toward
            # their class cluster simultaneously with the branch/feature weight
            # updates. Initialised at class means; regularised to stay nearby.
            self.centroids = nn.Parameter(centroids.clone())
            # Anchor = initial class means (frozen) — regularisation target
        ...
        def _distance_matrix(self, x, y):
            """
            L2 distance via torch.cdist. Caches centroid combined features
            once per epoch — centroids are learnable but don't change within
            a mini-batch, so recomputing them per batch is wasteful.
            Cache is invalidated after each optimizer step via the training loop.
            """
        ...
        def forward(self, x):
            """Pure AVM. Per-class tau broadcasts (N,K)/(K,) correctly."""
    ...
    def _loss(self, probs, targets, class_weights):
        ...
        # Centroid anchor — keep centroids near class means so they don't
        # drift into meaningless positions. Operates in raw normalised space
        # (before combined vector transformation) so the anchor is stable.
        c_anchor = self.centroid_reg * (
            self.net_.centroids - self.net_.centroid_anchor
        ).pow(2).mean()

        # Centroid separation — push centroids apart from each other.
        # Prevents collapse where all centroids converge to the same point.
        # Uses pairwise L2 distances in raw centroid space.
        if self.net_.K > 1:
            c_dists = torch.cdist(
                self.net_.centroids, self.net_.centroids, p=2)
            # Mean of upper triangle only (avoid self-distances and duplicates)
            mask    = torch.triu(torch.ones(self.net_.K, self.net_.K,
                                           device=c_dists.device), diagonal=1)
            c_sep   = -self.centroid_sep * (c_dists * mask).sum() / mask.sum()
        else:
    ...
    def fit(self, X, y):
        ...
        # Centroids — raw class means in normalised space.
        # James-Stein shrinkage is applied after training in combined space.
        ...
        # Pre-shuffle training tensors and use shuffle=False in DataLoader.
        # torch.randperm(N) inside DataLoader allocates an N-element int64
        # tensor each epoch — on 200k samples this triggers MemoryError on
        # memory-constrained machines. Pre-shuffling once with numpy (cheap)
        # and iterating sequentially avoids the per-epoch allocation.
        perm_idx = rng.permutation(len(X_tr_n))
        loader = torch.utils.data.DataLoader(
            torch.utils.data.TensorDataset(
                tt(X_tr_n[perm_idx]), tt(y_tr[perm_idx], long=True)),
        ...
        for epoch in range(self.epochs):
            # Re-shuffle each epoch with numpy — avoids torch.randperm(N)
            # which allocates an N-element int64 tensor and fails on large
            # datasets with limited memory.
            perm = rng.permutation(len(X_tr_n))
            loader.dataset.tensors = (X_tr_t[perm], y_tr_t[perm])
        ...
        # Store gravity weights — no longer used but kept for compatibility
        self.gravity_weights_ = np.ones(K, dtype=np.float32)
        ...
        def _make_combined(xn, batch_size=10_000):
            """
            Build combined vector in batches to avoid large contiguous
            allocation on memory-constrained machines.
            Full (N, 4F) array is assembled row-by-row from smaller chunks.
            """
            eps = 1e-7
            ...
            for start in range(0, len(xn), batch_size):
                ...
                phi_d = np.arccos(np.clip(chunk, -1+eps, 1-eps)) * sqfw
                phi_m = np.arccos(np.clip(np.abs(chunk), 0.0, 1-eps)) * sqfw
                euc   = np.tanh(np.arccos(
                    np.clip(chunk, -1+eps, 1-eps)) * 0.8) * sqfw
        ...
        # Centroids were learnable parameters — backprop moved them toward
        # their class clusters in normalised space. Convert to combined space
        # for AVM and FAISS inference using the final learned weights.
        # Shrinkage is no longer needed — the centroid anchor regularisation
        # played the same role during training (kept centroids near class
        # means while allowing them to find better positions).

        raw_centroids_n  = self.net_.centroids.detach().numpy()  # (K, F_n)
        X_tr_c_all       = _make_combined(X_tr_n)                # (N, 4F)
        self.centroids_combined_ = _make_combined(
            raw_centroids_n).astype(np.float32)                  # (K, 4F)

        # Compute shrinkage for display — how far each centroid moved
        # from its initial class mean (for inspection, not for correction)
        init_centroids_n = self.net_.centroid_anchor.numpy()     # (K, F_n)
        drift = np.sqrt(((raw_centroids_n - init_centroids_n)**2
                         ).sum(1))                               # (K,)
        self.centroid_drift_ = drift
        self.shrinkage_      = np.zeros(K, dtype=np.float32)     # legacy compat
        ...
        # ── Diagonal Mahalanobis variance ─────────────────────────────────
        # Estimate per-class variance in combined space. Using the corrected
        # centroid positions so variance is measured around the final centroid.
        # Diagonal only — avoids O(F²) estimation problems on minority classes.
        #
        # For each class k and each dimension d:
        #   var_k[d] = mean((x[d] - centroid_k[d])²) over class k samples
        #
        # Regularised with global variance to prevent division by near-zero
        # on dimensions where a class has very low spread.
        #   var_k_reg = (1 - reg) * var_k + reg * global_var
        #
        # mahal_reg=1e-6 is minimal — just prevents exact zeros.
        # Raise toward 0.1-0.5 on very small classes to borrow more global info.

        if self.mahalanobis:
            global_var = X_tr_c_all.var(0).astype(np.float32) + 1e-10
            class_vars = np.zeros((K, X_tr_c_all.shape[1]), dtype=np.float32)
            for i in range(K):
                cls_pts   = X_tr_c_all[y_tr == i]           # (n_k, 4F)
                centroid  = self.centroids_combined_[i]      # (4F,)
                # Variance measured around the shrinkage-corrected centroid
                sq_diff   = (cls_pts - centroid) ** 2       # (n_k, 4F)
                raw_var   = sq_diff.mean(0)                  # (4F,)
                # Regularise toward global variance
                class_vars[i] = ((1.0 - self.mahal_reg) * raw_var
                                 + self.mahal_reg * global_var)
            self.class_vars_ = class_vars                    # (K, 4F)
        else:
        ...
        if not _HAS_FAISS:
            ...
            # IVF nlist: FAISS k-means allocates O(N × nlist) internally.
            # Cap at sqrt(N) — a well-known heuristic that keeps cluster
            # granularity reasonable without blowing up memory.
            # Also enforce at least 39 points per cluster (FAISS requirement).
            built = False
            if self.use_ivf and N_tr >= 78:
                nlist = max(1, min(
                    self.nlist,
                    N_tr // 39,
                    int(np.sqrt(N_tr)),   # memory-safe cap
                ))
                try:
                    ...
                    built = True
                    if self.verbose:
                        print(f"  FAISS IVF: nlist={nlist} nprobe={index.nprobe}")
                except MemoryError:
                    if self.verbose:
            ...
            if not built:
                ...
                if self.verbose:
                    print(f"  FAISS Flat: {N_tr} vectors dim={dim}")

            index.add(X_tr_c)
            self.index_           = index
            self.train_class_idx_ = y_tr.copy()

            # Post-training lambda search on validation set.
            # Floor at 0.5 — below this, KNN weight exceeds AVM weight and
            # the blend is dominated by nearest-neighbour memorisation on
            # small imbalanced datasets. The AVM geometry should always
            # have at least equal influence in the final prediction.
            if self.auto_lambda:
                X_va_c  = _make_combined(X_va_n)
                avm_val = self._avm_proba(X_va_c)
                knn_val = self._knn_proba(X_va_c)

                lam_floor = getattr(self, 'lam_floor', 0.5)
                best_lam_f1, best_lam = -1.0, self.lam_init
                step = self.lam_search_step
                for lam_c in np.arange(lam_floor, 1.0 + step/2, step):
                ...
                if self.verbose:
                    print(f"  lambda search (floor={lam_floor}): "
                          f"best={best_lam:.2f}"
    ...
    def _avm_proba(self, combined):
        """
        AVM scoring. When mahalanobis=True uses diagonal Mahalanobis
        distance — each dimension scaled by per-class standard deviation.
        Falls back to Euclidean when mahalanobis=False.

        Diagonal Mahalanobis:
            d_k(x) = sqrt( sum_d( (x_d - μ_k_d)² / var_k_d ) )

        vs Euclidean:
            d_k(x) = sqrt( sum_d( (x_d - μ_k_d)² ) )

        The effect: dimensions where class k has high variance contribute
        less to the distance (the class is spread out there, so being far
        in that direction is expected). Dimensions where class k is tightly
        clustered contribute more (being far there is unusual for this class).
        """
        if self.class_vars_ is not None:
            # Diagonal Mahalanobis: (N, K) distance matrix
            # combined:      (N, 4F)
            # centroids:     (K, 4F)
            # class_vars_:   (K, 4F)  — per-class per-dim variance
            N   = combined.shape[0]
            K   = len(self.classes_)
            d   = np.zeros((N, K), dtype=np.float32)
            for k in range(K):
                diff    = combined - self.centroids_combined_[k]  # (N, 4F)
                scaled  = diff ** 2 / self.class_vars_[k]         # (N, 4F)
                d[:, k] = np.sqrt(scaled.sum(1))                  # (N,)
        else:
            # Standard Euclidean via matmul identity
    ...
    def _knn_proba(self, combined, search_batch=50_000):
        """
        FAISS KNN search with batching.

        Searching all N test points at once causes std::bad_alloc on large
        test sets — FAISS IVF allocates O(n × nlist) intermediate buffers.
        Batching caps peak memory at search_batch × nlist regardless of N.
        """
        ...
        for start in range(0, n_test, search_batch):
            ...
            t_idx = np.repeat(np.arange(batch_size), self.k)
    ...
    def summary(self, feature_names=None):
        ...
        lines = [
            ...
            f"  shrinkage_strength={self.shrinkage_strength}"
            f"  mahalanobis={self.mahalanobis}",
