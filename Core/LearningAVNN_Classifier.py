...
Optional dual‑anchor boundary branch: distances to both +e_f and -e_f.
"""
...
class LearningAVNN(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, k=5, lr=1e-3, epochs=300, batch_size=256,
                 ...
                 use_dual_boundary=False,   # <-- NEW toggle
        ...
        self.use_dual_boundary   = use_dual_boundary   # <-- NEW
    ...
    class _Net(nn.Module):
        ...
        def _boundary_dists(x, eps, dual=False):
            """
            Distances to face centres.
            If dual=False: return (N, F) distances to +e_f only.
            If dual=True:  return (N, 2F) distances to +e_f and -e_f (interleaved).
            """
            norm_sq = (x ** 2).sum(dim=1, keepdim=True)          # (N,1)
            dist_sq_pos = (norm_sq - 2.0 * x + 1.0).clamp(min=eps)  # (N,F)
            if not dual:
                return torch.sqrt(dist_sq_pos)
            else:
                dist_sq_neg = (norm_sq + 2.0 * x + 1.0).clamp(min=eps)  # (N,F)
                # Interleave positive and negative distances: [pos_f0, neg_f0, pos_f1, neg_f1, ...]
                dist_sq = torch.stack([dist_sq_pos, dist_sq_neg], dim=2)  # (N,F,2)
                dist_sq = dist_sq.view(dist_sq.shape[0], -1)               # (N,2F)
        ...
        def _triangle_score(self, x, proto_flat):
            ...
            x_dev  = x          - x.mean(1, keepdim=True)
            mu_dev = proto_flat - proto_flat.mean(1, keepdim=True)
            ...
            cross = (x_a.unsqueeze(1) * mu_b.unsqueeze(0)
                   - x_b.unsqueeze(1) * mu_a.unsqueeze(0))
            return torch.sqrt((w_pairs * cross ** 2).sum(-1) + eps)
    ...
    # ... (the rest of the class remains identical; copy from original,
    #  but ensure that in fit() we pass use_dual_boundary to _Net)
    # For brevity, the following methods are exactly as in the original
    # (loss, _val_score, fit, inference, summary) – except we add
    # use_dual_boundary when constructing self.net_.
    ...
    def _supcon_loss(self, embeddings, targets):
        ...
        log_prob = (sim - sim_max.detach()) - torch.log(exp_sim.sum(1, keepdim=True).clamp(min=1e-10))
    ...
    def _loss(self, probs, targets, class_weights, fw=None, embeddings=None):
        ...
        c_anchor = self.centroid_reg * (self.net_.centroids - self.net_.centroid_anchor).pow(2).mean()
        if self.net_._sep_cache is None:
            if self.net_.K > 1:
                ...
                if inter_mask.sum() > 0:
                    inter = -self.centroid_sep * (c_dists * inter_mask).sum() / inter_mask.sum()
                else:
                ...
                if m > 1 and self.intra_sep > 0.0:
                    for k in range(K):
                        ...
                        mask_k = torch.triu(torch.ones(m, m, device=d_intra.device), diagonal=1)
                        if mask_k.sum() > 0:
                            intra = intra - self.intra_sep * (d_intra * mask_k).sum() / mask_k.sum()
        ...
        if self.ortho_reg > 0.0 and embeddings is not None:
            ...
            chunks = embeddings[:, :n_branches * F_branch].reshape(-1, n_branches, F_branch)
    ...
    # ---- fit (only change: pass use_dual_boundary to _Net) ----
    def fit(self, X, y):
        ...
        if self.verbose and self.val_fraction == 0:
            print(f"  val_fraction=0: using {n_val} val samples ({n_val/len(X)*100:.1f}%)")
        ...
        proto_init = np.tile(class_means[:, None, :], (1, m, 1))
        if m > 1:
            jitter = rng_proto.normal(0, 0.05, proto_init.shape).astype(np.float32)
        ...
        self.net_ = self._Net(centroids, K, n_feat, m=m,
                              ...
                              use_dual_boundary=self.use_dual_boundary)   # <-- NEW
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(n_batch):
                ...
                self._loss(proba, yb, cw_t, fw=fw, embeddings=emb if self.supcon else None).backward()
                ...
                with torch.no_grad():
                    for cls in range(K):
                        ...
                        if m == 1:
                            ...
                            self.net_.centroids.data[cls, 0] = self.ema_beta * protos[0] + (1 - self.ema_beta) * cls_mean
                        else:
                            d_proto = torch.cdist(cls_X, protos, p=2)
                            resp = torch.softmax(-d_proto, dim=1)
                            for j in range(m):
                                w = resp[:, j:j+1]
                                w_sum = w.sum().clamp(min=1e-10)
                                wm = (w * cls_X).sum(0) / w_sum
                                self.net_.centroids.data[cls, j] = self.ema_beta * protos[j] + (1 - self.ema_beta) * wm
            ...
            if (epoch + 1) % val_every != 0 and epoch < self.epochs - 1:
                ...
                if self.verbose and (epoch+1)%10==0:
                    ...
                    print(f"  epoch {epoch+1:3d}  (val skipped) AVM[tac:{wa[0]:.2f} bnd:{wa[1]:.2f} cir:{wa[2]:.2f}] KNN[lin:{wk[0]:.2f} shp:{wk[1]:.2f} sq:{wk[2]:.2f}]")
            ...
            if self.verbose and (epoch+1)%10==0:
                wa = self.net_._avm_branch_weights().detach().numpy()
                wk = self.net_._knn_branch_weights().detach().numpy()
                print(f"  epoch {epoch+1:3d}  val_f1={vf1:.4f} AVM[tac:{wa[0]:.2f} bnd:{wa[1]:.2f} cir:{wa[2]:.2f}] KNN[lin:{wk[0]:.2f} shp:{wk[1]:.2f} sq:{wk[2]:.2f}]")
        ...
        # IMPORTANT: _make_avm_combined must use the same dual-boundary logic
        def _make_avm_combined(xn, batch_size=10_000):
            def _chunk(chunk):
                eps = 1e-7
                a = np.arccos(np.clip(chunk, -1+eps, 1-eps))
                tac = np.tanh(a * 0.8) * sqfw
                # Boundary – dual or positive-only
                norm_sq = (chunk**2).sum(1, keepdims=True)
                if self.use_dual_boundary:
                    dist_sq_pos = norm_sq - 2.0*chunk + 1.0
                    dist_sq_neg = norm_sq + 2.0*chunk + 1.0
                    bnd = np.sqrt(np.concatenate([dist_sq_pos, dist_sq_neg], axis=1).clip(min=eps))
                else:
                    bnd = np.sqrt((norm_sq - 2.0*chunk + 1.0).clip(min=eps))
                cir = np.sqrt(np.clip(1.0 - chunk*chunk, eps, None)) * sqfw
                return np.concatenate([sa_tac*tac, sa_b*bnd, sa_cir*cir], axis=1).astype(np.float32)
            if len(xn) <= batch_size:
                return _chunk(xn)
            return np.vstack([_chunk(xn[i:i+batch_size]) for i in range(0, len(xn), batch_size)])
        ...
        def _make_knn_combined(xn, batch_size=10_000):
            def _chunk(chunk):
                ...
                return np.concatenate([sk_lin*lin, sk_s*shp, sk_sq*sqd], axis=1).astype(np.float32)
            if len(xn) <= batch_size:
                return _chunk(xn)
            return np.vstack([_chunk(xn[i:i+batch_size]) for i in range(0, len(xn), batch_size)])
        ...
        raw_c_n = self.net_.centroids.detach().numpy()
        init_c_n = self.net_.centroid_anchor.numpy()
        raw_c_flat = raw_c_n.reshape(K * m, n_feat)
        self.centroids_combined_ = _make_avm_combined(raw_c_flat).astype(np.float32)
        self.n_prototypes_ = m
        drift_flat = np.sqrt(((raw_c_n - init_c_n)**2).sum(-1))
        self.centroid_drift_ = drift_flat.mean(1)

        counts_tr = np.array([(y_tr == i).sum() for i in range(K)], dtype=np.float32)
        if self.verbose:
            for i in range(K):
                proto_drifts = "  ".join(f"p{j}={drift_flat[i,j]:.4f}" for j in range(m))
                print(f"  centroid cls{self.classes_[i]}: n={int(counts_tr[i])}  drift=[{proto_drifts}]")
        ...
        if self.mahalanobis:
            ...
            for i in range(K):
                ...
                S_reg = ((1.0 - ridge) * S_blend + ridge * scale * np.eye(D))
                try:
                    L = np.linalg.cholesky(S_reg)
                    L_inv = np.linalg.solve(L, np.eye(D))
                    self.cov_inv_[i] = (L_inv.T @ L_inv).astype(np.float32)
                except np.linalg.LinAlgError:
                    self.cov_inv_[i] = np.linalg.pinv(S_reg).astype(np.float32)
            if self.verbose:
                print(f"  Mahalanobis: full RDA (alpha={alpha:.2f} ridge={ridge:.0e} D={D})")
        else:
            self.cov_inv_ = None

        self.lambda_ = self.lam_floor
        if not _HAS_FAISS:
            if self.verbose: print("  faiss not installed — KNN disabled, lambda=1.0")
            self.lambda_ = 1.0
            self.index_ = None
        else:
            ...
            if self.use_ivf and N_build >= 78:
                nlist = max(1, min(1000, N_build // 39, int(np.sqrt(N_build))))
                try:
                    ...
                    built = True
                    if self.verbose: print(f"  FAISS IVF: nlist={nlist} nprobe={index.nprobe}")
                except MemoryError:
                    if self.verbose: print("  FAISS IVF OOM — falling back to FlatL2")
            if not built:
                index = faiss.IndexFlatL2(dim)
                if self.verbose: print(f"  FAISS Flat: {N_build} vectors dim={dim}")
            index.add(X_tr_c)
            self.index_ = index
            ...
            if self.entropy_lambda:
                self.lambda_ = None
                if self.verbose: print(f"  lambda mode=entropy confident={self.lam_confident} uncertain={self.lam_uncertain}")
            else:
                ...
                if conf_f1 >= best_lam_f1:
                    self.lambda_ = None
                    if self.verbose: print(f"  lambda: confidence gate val_f1={conf_f1:.4f} (scalar={best_lam_f1:.4f})")
                else:
                    self.lambda_ = best_lam
                    if self.verbose: print(f"  lambda: scalar={best_lam:.2f} val_f1={best_lam_f1:.4f} (conf={conf_f1:.4f})")
    ...
    def _avm_proba(self, combined):
        ...
        if self.cov_inv_ is not None:
            ...
            d_flat = np.sqrt(np.maximum(a_sq + b_sq[None, :] - 2.0 * ab, 0.0)).astype(np.float32)
    ...
    def _knn_proba(self, combined, search_batch=50_000):
        ...
        for start in range(0, n_test, search_batch):
            ...
            np.add.at(proba[start:end], (t_idx, self.train_class_idx_[idx.ravel()]), w.ravel())
        return proba
    ...
    def predict_proba(self, X):
        ...
        if self.index_ is None or (self.lambda_ is not None and self.lambda_ >= 1.0):
            return avm
        knn = self._knn_proba(self._make_knn_combined(Xn))
        if self.entropy_lambda:
            K = len(self.classes_)
            H = -(avm * np.log(avm.clip(1e-10))).sum(1) / np.log(K)
            lam = self.lam_confident - (self.lam_confident - self.lam_uncertain) * H
            lam = lam.clip(self.lam_uncertain, self.lam_confident)
    ...
    def summary(self, feature_names=None):
        ...
        names = (list(feature_names) if feature_names is not None else [f"f{i}" for i in range(len(self.feat_weights_))])
        bw = self.branch_weights_
        lam = getattr(self, 'lambda_', self.lam_floor)
        tau = self.tau_
        tau_str = (f"{float(tau):.3f}" if np.ndim(tau)==0 or np.size(tau)==1 else "[" + " ".join(f"cls{i}:{v:.3f}" for i,v in enumerate(tau)) + "]")
        lam_str = (f"{lam:.3f}" if lam is not None else "[entropy]")
        lines = [
            f"  tau={tau_str}  lambda={lam_str}  entropy_lambda={self.entropy_lambda}",
            f"  ordinal={self.ordinal}  per_class_tau={self.per_class_tau}",
            f"  mahalanobis={'full' if self.mahalanobis else 'off'}  alpha={self.mahal_alpha}  ortho_reg={self.ortho_reg}",
            f"  supcon={self.supcon}  ema_centroids={self.ema_centroids}  n_prototypes={getattr(self, 'n_prototypes_', self.n_prototypes)}  triangle={self.use_triangle}",
            f"  AVM branch weights: tanh_arccos={bw[0]:.3f}  boundary={bw[1]:.3f}  circular={bw[2]:.3f}  (dual_boundary={self.use_dual_boundary})",
            f"  KNN branch weights: linear={bw[3]:.3f}  shape={bw[4]:.3f}  quadratic={bw[5]:.3f}",
        ]
        ...
        if hasattr(self, 'centroid_drift_') and self.centroid_drift_ is not None:
            d_str = "  ".join(f"class{c}={v:.4f}" for c,v in zip(self.classes_, self.centroid_drift_))
            lines.append(f"  Centroid drift: {d_str}")
        if hasattr(self, 'branch_frozen_') and self.branch_frozen_.any():
            bnames = ['tac','bnd','cir','lin','shp','sq']
