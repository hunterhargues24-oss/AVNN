"""
FastAVNNClassifier – FAISS‑accelerated KNN with fully vectorised prediction.
"""
...
class FastAVNNClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, alpha=0.5, lam=0.7, k=5, transform='tanh_ac',
                 norm_range='[-1,1]', eps=1e-10, nprobe=10, use_ivf=True, nlist=1000):
        self.alpha = alpha
        self.lam = lam
        self.k = k
    ...
    def _make_combined_vector(self, X_norm, X_trans):
        ...
        euc = X_trans
        scale_ang = np.sqrt(self.alpha)
        scale_euc = np.sqrt(1.0 - self.alpha)
        return np.concatenate([scale_ang * ang, scale_euc * euc], axis=1)
    ...
    def fit(self, X, y):
        ...
        # Precompute class index mapping for fast voting
    ...
    def predict_proba(self, X):
        ...
        # Vectorised weighted voting using np.add.at
